    ...
    const handleSendMessage = async (message) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const lastAssistantMsg = rawHistory.findLast(m => m.role === "assistant");

                if (lastAssistantMsg) {
                    if (stems.length > 0) lastAssistantMsg.stems = stems;
                    if (remix) lastAssistantMsg.remix = remix;

                    setChatHistory(prev => [...prev, lastAssistantMsg]);
                }
